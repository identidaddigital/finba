...
function cf7msm_multiform_shortcode_handler( $tag ) {
    ...
    if ( cf7msm_fs()->is__premium_only() ) {
        if ( empty( $cf7msm_posted_data ) || empty( $cf7msm_posted_data['cf7msm-no-ss'] ) ) {
            $ret = sprintf( '<span id="cf7msm_%s" class="cf7msm-ro"></span>', $field_name );
            //DO NOT REMOVE THE SPAN ID!
            return apply_filters('cf7msm_form_field_value_pro', $ret, $field_name, $id_att );
        }
    }
...
;
    ...
    $value = apply_filters_deprecated( 'wpcf7_form_field_value', array( apply_filters_deprecated( 'wpcf7_form_field_value_'.$id_att, array( $value ), '3.0.4', 'cf7msm_form_field_value_'.$id_att ), $field_name, $value ), '3.0.4', 'cf7msm_form_field_value' );

    $value = apply_filters('cf7msm_form_field_value', apply_filters( 'cf7msm_form_field_value_'.$id_att, $value ), $field_name, $id_att, $value );
    return $value;
}
